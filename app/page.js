...
{/* Header */} <header className="flex flex-col items-center justify-start pt-15 w-full bg-green-700 text-white fixed top-0 left-0 z-20"> <div className="flex items-center gap-2"> <img
       ...
       /> <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-green-100">Stable Quiz</h1> </div> </header>


  {/* Main */}
  <main className="flex-1 flex items-center justify-center pt-28 pb-24 w-full px-2">
    {!started ? (
      <div className="bg-white/90 rounded-3xl p-6 md:p-10 shadow-xl w-full max-w-md text-center animate-slide-up transition-all duration-500">
        <h2 className="text-2xl font-semibold text-green-800 mb-2">Welcome to the Stable Quiz!</h2>
        <p className="text-gray-700 mb-6 text-sm md:text-base leading-relaxed">
          Test your knowledge of Stable blockchain and USDT features. 10 challenging questions await.
        </p>
        <button onClick={() => setStarted(true)} className="px-6 py-2 bg-gradient-to-r from-green-500 to-green-700 text-white rounded-full shadow hover:scale-105 transition-transform font-medium">
          Start Quiz
        </button>
      </div>
    ) : !finished ? (
      <div className="bg-white/90 p-10 rounded-3xl p-6 md:p-10 shadow-xl w-full max-w-md">
        ...
        <div className="sm:text-[16px] grid grid-cols-1 gap-3">
          {questions[currentIndex].options.map((option, idx) => {
            ...
            if (selectedOption) {
              if (option === questions[currentIndex].answer) bgColor = "bg-green-500 text-white transition-colors duration-300";
              else if (option === selectedOption && option !== questions[currentIndex].answer) bgColor = "bg-red-400 text-white transition-colors duration-300";
            }
            return (
              <button
                key={idx}
      ...
      <div className="bg-white/90 rounded-3xl p-8 md:p-12 shadow-xl text-center w-full max-w-md animate-slide-up">
        <h2 className="text-2xl font-bold mb-2 text-green-800">Quiz Completed!</h2>
        <p className="text-base mb-2">Your Score: <span className="font-bold">{score}</span> / {questions.length}</p>
        <p className=" mb-4 "> Great job! Keep learning and exploring Stable blockchain features!</p>
        <button onClick={handleRestart} className="px-6 py-2 bg-gradient-to-r from-green-500 to-green-700 text-white rounded-full shadow hover:scale-105 transition-transform font-medium">
           Restart Quiz
        </button>
      </div>
    )}
  ...
  {/* Footer */}
  <footer className="fixed bottom-0 w-full py-2 text-center bg-green-700 text-white ">
